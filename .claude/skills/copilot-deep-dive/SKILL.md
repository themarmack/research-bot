...
description: On-demand Category 1 researcher focused on GitHub Copilot — enterprise features, models, admin controls, data handling, content exclusions, IP indemnity, agentic features, AGENTS.md governance, Copilot CLI/Chat/Workspace, and Copilot in regulated-environment contexts. Enforces the Obsidian-first contract: vault-querier first against vault/facts/copilot/ and vault/research/copilot/, then web research only for confirmed gaps. Findings land at vault/research/copilot/YYYY-MM-DD-{slug}.md; verified load-bearing claims get staged to _inbox/ for memory-curator to promote to vault/facts/copilot/. Use when the user asks to research, deep-dive, or investigate a GitHub Copilot topic **in depth** — including IP indemnity, content exclusions, data handling, admin controls, models, or agentic features — producing a fresh, cited research note — as opposed to a one-line canonical answer to a recurring policy question, which routes to copilot-faq-answerer.
---
